...
export default function AdminDashboard() {
  ...
  const fetchDashboardData = async () => {
    ...
    try {
      // Fetch stats
      const statsResponse = await documentAPI.getAdminStats();
      ...
      const requestsResponse = await documentAPI.getAdminRequests(params);
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
          <div className="flex justify-between items-center">
            <div>
              ...
              <p className="text-gray-600 mt-1">Manage document requests and approvals</p>
            </div>
            <button
          ...
          <div className="flex flex-wrap gap-4 items-end">
            ...
            <div>
              ...
              <select
                ...
                onChange={(e) => setFilter({ ...filter, documentType: e.target.value, currentPage: 1 })}
                className="px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500 min-w-[160px]"
              >
                <option value="">All Types</option>
                <option value="marriage-register">Marriage Register</option>
                <option value="single-girl">Single Girl Child</option>
                <option value="signature">Signature Change</option>
                <option value="name-change">Name Change</option>
                <option value="rental-agreements">Rental Agreements</option>
                <option value="short-attendence">Short Attendance</option>
              </select>
            </div>
            
            <button
              onClick={fetchDashboardData}
              className="px-6 py-2 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 transition mt-6"
            >
              Apply Filters
            </button>
          </div>
                  ...
                  requests.map((request) => (
                    <tr key={request._id} className="hover:bg-gray-50 transition">
                      ...
                      <td className="px-6 py-4">
                        <div>
                          <p className="text-sm font-medium text-gray-900">{request.userId?.name || 'N/A'}</p>
