...
export default function NameAdditionBirthCertificate() {
  ...
  const fetchServiceCharge = async () => {
    ...
    try {
      const response = await serviceChargeAPI.getChargeByDocumentType('name-addition-birth-certificate');
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const uploadResult = await uploadPDFToCloudinary(pdfBlob, 'name-addition-birth-certificate', requestId);
        ...
        <div className="bg-gray-100 rounded-xl shadow overflow-auto flex justify-center p-4" style={{ height: "90vh" }}>
          ...
          >
            ...
            <div style={{ marginLeft: "0px" }}>
              ...
              <p style={{ margin: "10px 0" }}>
                2. That I/My wife delivered a <b>{data.childGender || "__________"}</b> baby child on <b>{formatDateForDisplay(data.birthDate)}</b> at <b>{data.birthPlace || "__________"}</b>.
              </p>

              <p style={{ margin: "10px 0" }}>
                3. That my son/daughter Birth Certificate Registration Number is <b>{data.certificateNumber || "__________"}</b> dated <b>{formatDateForDisplay(data.certificateDate)}</b>.
              </p>

              <p style={{ margin: "10px 0" }}>
                4. That I want to include my child's name in his/her Birth Certificate.
              </p>
