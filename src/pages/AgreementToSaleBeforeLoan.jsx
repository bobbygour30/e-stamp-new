...
export default function AgreementToSaleBeforeLoan() {
  ...
  useEffect(() => {
    if (data.agreementDate && data.balancePaymentMonths) {
      const startDate = new Date(data.agreementDate);
      const endDate = new Date(startDate);
      endDate.setMonth(endDate.getMonth() + parseInt(data.balancePaymentMonths));
      const formattedEndDate = endDate.toISOString().split('T')[0];
      setData(prev => ({ ...prev, balancePaymentEndDate: formattedEndDate }));
    }
  }, [data.agreementDate, data.balancePaymentMonths]);
      ...
      <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white p-6 rounded-xl shadow border border-purple-200 overflow-y-auto" style={{ maxHeight: "90vh" }}>
          ...
          <div className="bg-yellow-50 p-3 rounded-lg mb-4">
            ...
            <div className="grid grid-cols-2 gap-3">
              <Input label="Balance Payment Time (Months)" name="balancePaymentMonths" type="number" value={data.balancePaymentMonths} onChange={update} />
          ...
          <div className="bg-orange-50 p-3 rounded-lg mb-4">
            ...
            <textarea
              ...
              placeholder="Enter payment details (e.g., Banking OCR Rekha to Rupam - 50k on 19/08, etc.)"
            />
          </div>
        ...
        <div className="bg-gray-100 rounded-xl shadow overflow-auto flex justify-center p-4" style={{ height: "90vh" }}>
          ...
          >
            ...
            {/* Seller and Purchaser Section */}
            <p style={{ textAlign: "justify", marginBottom: "12px" }}>
              This Sale Agreement is executed at <b>New Delhi</b> on this <b>{formatDateForDisplay(data.agreementDate)}</b>, between <b>{data.sellerName || "MRS. RUPAM DEVI"}</b> (AADHAR CARD NO. <b>{data.sellerAadhaar || "6306 5660 0255"}</b>) {data.sellerRelation || "W/O"} <b>{data.sellerRelationName || "MR. MUKESH SINGH"}</b> R/O <b>{data.sellerAddress || "H. NO.RC-193, ADARSH NAGAR, F BLOCK, KHORA, GHAZIABAD, UTTAR PRADESH-201309"}</b>, (hereinafter referred to as to <b>SELLER / FIRST PARTY</b>, which term and administrator) of the one part.
            </p>

            <p style={{ textAlign: "justify", marginBottom: "12px" }}>
              AND <b>{data.purchaserName || "MRS REKHA DEVI"}</b> {data.purchaserRelation || "W/O"} <b>{data.purchaserRelationName || "MR. AMIT KUMAR"}</b> R/O <b>{data.purchaserAddress || "KHASRA NO.232, KRISHNA NAGAR BAGU, VIJAY NAGAR, NEAR ROSE VALLEY PUBLIC SCHOOL, VIJAY NAGAR, GHAZIABAD, UTTAR PRADESH-201009"}</b> (hereinafter referred to as the <b>PURCHASER / SECOND PARTY</b>), which terms assigns, executors, successors, legal representatives and administrators) of the other part.
            </p>
            ...
            <div style={{ marginLeft: "0px" }}>
              ...
              {/* Payment Details Text */}
              <div style={{ marginLeft: "20px", marginBottom: "10px", whiteSpace: "pre-line" }}>
                {data.paymentDetails ? (
                  <p style={{ margin: "4px 0" }}>{data.paymentDetails}</p>
                ) : (
                  <p style={{ margin: "4px 0" }}>Banking OCR Rekha to Rupam - Total ₹2,15,000 (19/08: 50k, 30/09: 20k, 05/10: 30k, 26/10: 25k, 28/11: 20k+20k+50k)</p>
                )}
